...
| Category suggestions (`org.jevintegration.category`) | TypeSafe's hierarchical classification: a Choice per level, 3 paths kept (beam search), one request per level. Records a suggestion with its path and score and never changes a product on its own: a merchandiser applies or dismisses it with the Backoffice actions (`JevSuggestionService`). Cronjobs `jevCategoryDryRunCronJob` (products that have a category, to compare) and `jevCategorySuggestionCronJob` (products without one). Configure `jev.category.catalog`, `jev.category.roots`, `jev.category.min.score`. |
| Attribute suggestions (`org.jevintegration.attribute`) | For each enum attribute of the product's classification class, one Choice over its allowed values plus "not stated", all attributes in one request. Records one suggestion per product and never changes feature values on its own; Apply sets only attributes that are still empty. Cronjobs `jevAttributeDryRunCronJob` and `jevAttributeSuggestionCronJob`. Configure `jev.attribute.catalog`, `jev.attribute.system`, `jev.attribute.min.confidence`. |
